...
# Crea tarjeta de título con audio silencioso para facilitar el concat
make_title() {
  local label="$1"
  local out="$2"
  ffmpeg -y \
    ...
    -vf "drawtext=fontfile=${FONTFILE}:text='sight reading exercise (${label})':fontcolor=white:fontsize=64:x=(w-text_w)/2:y=(h-text_h)/2" \
    -c:v libx264 -preset medium -crf 18 -r 30 -pix_fmt yuv420p \
    -c:a aac -b:a 192k -ar 48000 -ac 2 \
    -shortest "$out"
}

# Combina imagen de partitura a pantalla completa + PiP del vídeo en esquina inferior derecha
make_pair() {
  local img="$1"
  local vid="$2"
  local out="$3"
  ffmpeg -y -loop 1 -i "$img" -i "$vid" \
    -filter_complex "[0:v]scale=${WIDTH}:${HEIGHT}:force_original_aspect_ratio=decrease,pad=${WIDTH}:${HEIGHT}:(ow-iw)/2:(oh-ih)/2:black[bg];[1:v]scale=${PIP_W}:-2:flags=lanczos[small];[bg][small]overlay=x=W-w-${MARGIN}:y=H-h-${MARGIN}:format=auto,format=yuv420p[v]" \
    -map "[v]" -map 1:a? \
    -c:v libx264 -preset medium -crf 18 -r 30 -pix_fmt yuv420p \
    -c:a aac -b:a 192k -ar 48000 -ac 2 \
    -shortest "$out"
...
# Concatena sin re-codificar
ffmpeg -y -f concat -safe 0 -i inputs.txt -c copy "$OUT"
